...
/***/ (function(module, exports) {
	...
	var config = {
	  apiOrigins: {
	    production: 'https://octater-hangman.herokuapp.com'
	  }
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var player1 = __webpack_require__(9);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var gamePhrase = __webpack_require__(13);
	...
	module.exports = {
	  resetAlphabetKeys: resetAlphabetKeys,
	  removeLetters: removeLetters,
	  removeBlanksAroundWord: removeBlanksAroundWord,
	  setWord: setWord
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var onPlayGame = function onPlayGame(event) {
	  ...
	  if (player1.user.id === 0) {
	    ...
	    var title = 'ERROR';
	    var body = 'Player must be signed in before creating a new game';
	    $('#alert-modal-title').html(title);
	    $('#alert-modal-body').html(body);
	    $('#alert-modal').modal('show');
	    return;
	  ...
	  api.getPhrase().done(ui.getPhraseSuccess).fail(ui.getPhraseFailure);
	};
	...
	var onMyStats = function onMyStats() {
	  ...
	  if (player1.user.id === 0) {
	    ...
	    var title = 'ERROR';
	    var body = 'Player must be signed in before getting their stats';
	    $('#alert-modal-title').html(title);
	    $('#alert-modal-body').html(body);
	    $('#alert-modal').modal('show');
	    return;
	  ...
	  api.myStats().done(ui.myStatsSuccess).fail(ui.myStatsFailure);
	};

	var purgeMyStats = function purgeMyStats() {
	  // event.preventDefault()
	  ...
	  if (player1.user.id === 0) {
	    // console.log('user player1 requested game stats ', player1)
	    var title = 'ERROR';
	    var body = 'Player must be signed in before purging their games';
	    $('#alert-modal-title').html(title);
	    $('#alert-modal-body').html(body);
	    $('#alert-modal').modal('show');
	    return;
	  ...
	  if (game.gameOver === 'N') {
	    // console.log('user player1 requested game stats ', player1)
	    var _title = 'ERROR';
	    var _body = 'Please finish current game prior to deleting games.';
	    $('#alert-modal-title').html(_title);
	    $('#alert-modal-body').html(_body);
	    $('#alert-modal').modal('show');
	    return;
	  }
	  // console.log('made it to my stats logic: ', player1)

	  api.myStats().done(ui.purgeMyStatsSuccess).fail(ui.purgeMyStatsFailure);
	};
	...
	function pickLetter() {
	  if (player1.user.id === 0) {
	    // console.log('user player1 requested new game ', player1)
	    var title = 'ERROR';
	    var body = 'Player must be signed in before playing a game';
	    $('#alert-modal-title').html(title);
	    $('#alert-modal-body').html(body);
	    $('#alert-modal').modal('show');
	    return;
	...
	function handlePickedLetter(letterPicked) {
	  ...
	  var ind = gamePhrase.content.phrase_content.indexOf(letterPicked);
	...
	function displayCongratulatoryMessageOnWin() {
	  ...
	  if (correctlyGuessedLettersCount === gamePhrase.content.phrase_content.length) {
	    ...
	    var gameWinnerMessage = "Congratulations, you've won the game in only - '" + game.lettersPlayed + "' letters";
	...
	function displayGameOverMessageOnLose() {
	  ...
	  if (game.bodyParts === 7) {
	    ...
	    var gameOverMessage = "You took too many tries to guess the word. The correct word is - '" + gamePhrase.content.phrase_content + "'";
	...
	var updtGame = function updtGame() {
	  // event.preventDefault()
	  // const data = getFormFields(event.target)
	  api.updateMove().done(ui.updateMoveSuccess).fail(ui.updateMoveFailure);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var purgeGame = function purgeGame(data) {
	  return $.ajax({
	    method: 'DELETE',
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var playGameFailure = function playGameFailure(error) {
	...
	var getPhraseSuccess = function getPhraseSuccess(data) {
	  ...
	  api.playGame().done(playGameSuccess).fail(playGameFailure);
	};

	var getPhraseFailure = function getPhraseFailure(error) {
	  // console.error('getPhraseFailiure log', error)
	  var title = 'ERROR';
	  var body = 'Error with getting phrase for the game -- ' + error.statusText;
	  $('#alert-modal-title').html(title);
	  $('#alert-modal-body').html(body);
	  $('#alert-modal').modal('show');
	};
	...
	var purgeMyStatsFailure = function purgeMyStatsFailure(error) {
	  // console.error('myStatsFailiure log', error)
	  var title = 'ERROR';
	  var body = 'Error with purging your stats.  Sorry bud -- ' + error.statusText;
	  $('#alert-modal-title').html(title);
	  $('#alert-modal-body').html(body);
	  $('#alert-modal').modal('show');
	};

	var purgeGameSuccess = function purgeGameSuccess(data) {
	  // console.log('here is the result of myStatsSuccess ', data)
	};

	var purgeGameFailure = function purgeGameFailure(error) {
	  // console.error('myStatsFailiure log', error)
	  var title = 'ERROR';
	  var body = 'Error with deleting game.  Sorry bud -- ' + error.statusText;
	  $('#alert-modal-title').html(title);
	  $('#alert-modal-body').html(body);
	  $('#alert-modal').modal('show');
	};

	var getRecord = function getRecord(data) {
	...
	var purgeGames = function purgeGames(data) {
	  var games = data.games;
	  // console.log('here is the const game ', games)
	  // const obj = {}
	  var gamesToDelete = games.length;
	  ...
	  games.forEach(function (item) {
	    ...
	    api.purgeGame(item.id).done(purgeGameSuccess).fail(purgeGameFailure);
	  });

	  var finalRecord = 'You have just deleted ' + gamesToDelete + ' games';
	  return finalRecord;
	};
	...
	module.exports = {
	  ...
	  getPhraseSuccess: getPhraseSuccess,
	  getPhraseFailure: getPhraseFailure,
	  ...
	  purgeMyStatsSuccess: purgeMyStatsSuccess,
	  purgeMyStatsFailure: purgeMyStatsFailure,
	  purgeGameSuccess: purgeGameSuccess,
	  purgeGameFailure: purgeGameFailure,
	  getRecord: getRecord
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
